...
void Bitmap::transform(const Transformer& m) {
    Display* d = Session::instance()->default_display();
    DisplayRep* r = d->rep();
    XDisplay* dpy = r->display_;
    BitmapRep* b = rep();
    ...
    m.transform(b->left_, b->bottom_, x1, y1);
    m.transform(b->left_, b->top_, x2, y2);
    m.transform(b->right_, b->top_, x3, y3);
    m.transform(b->right_, b->bottom_, x4, y4);
...
void Bitmap::Transform(const Transformer* t) { transform(*t); }
...
BitmapRep::BitmapRep(BitmapRep* b, unsigned int t) {
    ...
    b->fill();
    fill();
    for (register int x = 0; x < pwidth_; ++x) {
	for (register int y = 0; y < pheight_; ++y) {
            unsigned long pixel;
	    int bx, by;
            switch (t) {
            case copy:   bx = x; by = y; break;
	    case fliph:  bx = pwidth_ - x - 1; by = y; break;
	    case flipv:  bx = x; by = pheight_ - y - 1; break;
	    case rot90:  bx = pheight_ - y - 1; by = x; break;
	    case rot180: bx = pwidth_ - x - 1; by = pheight_ - y - 1; break;
	    case rot270: bx = y; by = pwidth_ - x - 1; break;
            case inv:    bx = x; by = y; break;
            }
	    ...
	    XPutPixel(image_, x, pheight_ - y - 1, pixel);
	}
    }
}
