...
boolean MultiLineObj::Intersects(LineObj& l) {
    ...
    if (b.Intersects(l)) {
	for (i = 1; i < count; ++i) {
	    LineObj test(x[i-1], y[i-1], x[i], y[i]);
...
boolean FillPolygonObj::Contains (PointObj& p) { // derived from A. Glassner,
  if (normCount == 0) {                          // "An Introduction to
      Normalize();                               // Ray Tracing", p. 53,
  }                                              // courtesy R. Cooperman

  int count = 0;
  PointObj p0(0, 0);
  boolean cur_y_sign = normy[0] >= p.y;

  for (int i = 0; i < normCount - 2; ++i) {
      LineObj l (
          normx[i] - p.x, normy[i] - p.y,
          normx[i+1] - p.x, normy[i+1] - p.y
      );
      ...
      boolean next_y_sign = l.p2.y >= 0;

      if (next_y_sign != cur_y_sign) {
          boolean cur_x_sign = l.p1.x >= 0;
          boolean next_x_sign = l.p2.x >= 0;

          if (cur_x_sign && next_x_sign) {
              ++count;

          } else if (cur_x_sign || next_x_sign) {
              Coord dx = l.p2.x - l.p1.x;
              Coord dy = l.p2.y - l.p1.y;

              if (dy >= 0) {
                  if (l.p1.x * dy > l.p1.y * dx) {
                      ++count;
                  }
              } else {
                  if (l.p1.x * dy < l.p1.y * dx) {
...
boolean FillPolygonObj::Intersects(BoxObj& ub) {
    ...
    LineObj bottom(ub.left, ub.bottom, ub.right, ub.bottom);

    if (Intersects(bottom)) {
	return true;
    }
    LineObj right(ub.right, ub.bottom, ub.right, ub.top);

    if (Intersects(right)) {
	return true;
    }
    LineObj top(ub.right, ub.top, ub.left, ub.top);

    if (Intersects(top)) {
	return true;
    }
    LineObj left(ub.left, ub.top, ub.left, ub.bottom);
...
void Extent::Merge(Extent& e) {
    float nl = min(left, e.left);
    float nb = min(bottom, e.bottom);

    if (Undefined()) {
	left = e.left; bottom = e.bottom; cx = e.cx; cy = e.cy;
    } else if (!e.Undefined()) {
	cx = (nl + max(2*cx - left, 2*e.cx - e.left)) / 2;
	cy = (nb + max(2*cy - bottom, 2*e.cy - e.bottom)) / 2;
	left = nl;
	bottom = nb;
    }
    tol = max(tol, e.tol);
