#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/tif_compat.c,v 1.9 91/07/16 16:30:48 sam Exp $";
#endif
	...
	long *psize;
{
	...
	if (size != -1) {
		*pbase = (u_char *) mmap(0, size, PROT_READ, MAP_SHARED, fd, 0);
		if (*pbase != (u_char *)-1) {
			*psize = size;
			return (1);
		}
