...
Label::Label (const char* s, Graphic* gr) : Graphic(gr) {
    _font = nil;
    if (gr != nil) {
...
void Label::SetFont (PSFont* font) {
    if (_font != font) {
        Ref(font);
        Unref(_font);
	_font = font;
...
PSPattern* Label::GetPattern () { return pssolid; }

PSFont* Label::GetFont () { return _font; }
Graphic* Label::Copy () { return new Label(_string, this); }
