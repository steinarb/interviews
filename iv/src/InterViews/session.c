...
Session* SessionRep::instance_ = nil;
...
// added by ro2m: this is necessary for running your own event processing loop..
void Session::undone() {
    rep_->done_ = false;
}
...
void Session::read(Event& e, boolean (*test)()) {
...
void SessionRep::set_style(Display* d) {
    ...
    load_path(s, ABSLIBALLDIR, "/InterViews", -5);
    load_path(s, RELLIBALLDIR, "/InterViews", -5);
...
void SessionRep::load_app_defaults(Style* s, int priority) {
    ...
    load_path(s, ABSLIBALLDIR, "/", classname_, priority);
    load_path(s, RELLIBALLDIR, "/", classname_, priority);
