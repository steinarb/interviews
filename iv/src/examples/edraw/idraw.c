...
IdrawReaderImpl::IdrawReaderImpl () {}

IdrawReaderImpl::~IdrawReaderImpl () {
    for (ListItr(BrushInfoList) i(brushes_); i.more(); i.next()) {
	BrushInfo& br = i.cur_ref();
        Resource::unref(br.brush_);
    }
    for (ListItr(StippleList) j(stipples_); j.more(); j.next()) {
	const Stipple* st = j.cur();
        Resource::unref(st);
    }
}

Graphic* IdrawReader::load(InputFile* f) {
...
Graphic* IdrawReader::load(const char* filename) {
    Graphic* gm = nil;
    String s(filename);
...
Graphic* IdrawReaderImpl::load(
	...
	String figname(fig->name);
        if (fig->name == nil) {
            ...
            PolyGraphic* pg = new PolyGraphic;
            pg->transformer(tx);
	    for (;;) {
