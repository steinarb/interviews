...
implementPtrList(PacerList, Pacer);
implementPtrList(SchedulerList, Scheduler);

FadeType fadetype (Grade cur, Grade fut, Grade low, Grade high) {
    if (cur != fut) {
        if (cur >= low && fut < low) {
            return fadeout_low;
        } else if (cur < low  && fut >= low) {
            return fadein_low;
        } else if (cur > high && fut <= high) {
            return fadein_high;
        } else if (cur <= high && fut > high) {
            return fadeout_high;
        } else if (cur >= low && cur <= high && fut >= low && fut <= high) {
            return sustain;
        } else {
            return no_fade;
        }            
...
TimeSlice::TimeSlice (USec nat, USec str, USec shr) {
    _nat = nat;
    _str = str;
    _shr = shr;
}

TimeSlice::TimeSlice (const TimeSlice& s) {
    _nat = s._nat;
    _str = s._str;
    _shr = s._shr;
}

TimeSlice& TimeSlice::operator = (const TimeSlice& s) {
    _nat = s._nat;
    _str = s._str;
    _shr = s._shr;
    return *this;
...
boolean TimeSlice::operator == (const TimeSlice& s) {
    return (_nat == s._nat && _str == s._str && _shr == s._shr);
}
...
USec TimeSlice::min () { return (_nat-_shr); }
USec TimeSlice::max () { return (_nat+_str); }
...
BoxObj::BoxObj (Coord x0, Coord y0, Coord x1, Coord y1) {
    _l = min(x0, x1); _b = min(y0, y1); 
    _r = max(x0, x1); _t = max(y0, y1);
}

BoxObj::BoxObj (BoxObj* b) {
    _l = b->_l; _b = b->_b; _r = b->_r; _t = b->_t;
}

boolean BoxObj::operator== (BoxObj& box) {
    float tol = 0.0001;
    return ( 
        Math::equal(_l, box._l, tol) && 
        Math::equal(_r, box._r, tol) &&
        Math::equal(_b, box._b, tol) && 
        Math::equal(_t, box._t, tol)
    );
}
...
boolean BoxObj::Intersects (LineObj& l) {
	...
	intersects = Contains(l._p1) || Contains(l._p2);
        if (!intersects) {
            LineObj l0 (_l, _b, _r, _b);
            LineObj l1 (_r, _b, _r, _t);
            LineObj l2 (_r, _t, _l, _t);
            LineObj l3 (_l, _t, _l, _b);
            intersects =
...
boolean FillPolygonObj::Intersects (BoxObj& ub) {
    ...
    LineObj bottom(ub._l, ub._b, ub._r, ub._b);

    if (Intersects(bottom)) {
	return true;
    }

    LineObj right(ub._r, ub._b, ub._r, ub._t);

    if (Intersects(right)) {
	return true;
    }

    LineObj top(ub._r, ub._t, ub._l, ub._t);

    if (Intersects(top)) {
	return true;
    }

    LineObj left(ub._l, ub._t, ub._l, ub._b);
...
Extent::Extent (float x0, float y0, float x1, float y1, float t) {
    _l = x0; _b = y0; _cx = x1; _cy = y1; _tol = t;
}
...
boolean Extent::Within (Extent& e) {
    float l = _l - _tol, b = _b - _tol;
    float el = e._l - _tol, eb = e._b - _tol;
...
void Extent::Merge (Extent& e) {
    float nl = min(_l, e._l);
    float nb = min(_b, e._b);

    if (Undefined()) {
	_l = e._l; _b = e._b; _cx = e._cx; _cy = e._cy;
    } else if (!e.Undefined()) {
	_cx = (nl + max(2*_cx - _l, 2*e._cx - e._l)) / 2;
	_cy = (nb + max(2*_cy - _b, 2*e._cy - e._b)) / 2;
	_l = nl;
	_b = nb;
    }
